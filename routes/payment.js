...
router.post("/pay", (req, res) => {
  ...
  if (
    !req.body.shipping.firstName ||
    !req.body.shipping.lastName ||
    ...
    return res.status(400).send({
      message:
        "Something went wrong. Make sure you've filled out all required fields.",
    });
  }
  ...
  if (req.body.coins.length <= 0) {
    ...
    return res.status(400).send({
      message:
        "Something went wrong. Make sure you've filled out all required fields.",
      error: "Trying to purchase 0 coins.",
    });
  }
  ...
  db.query(`SELECT * FROM coins WHERE id in (${idsQuery})`, (err, rows) => {
    ...
    coins.forEach((coin) => {
      ...
      items.push({
        name: coin.name,
        price: (coin.price / 100).toLocaleString(undefined, {
    ...
    if (subtotal < minPurchase) {
      ...
      return res.status(400).send({
        message:
          "Something went wrong. Make sure you've filled out all required fields.",
        error: `You must purchase at least $${minPurchase} worth of product.`,
      });
    }
    ...
    console.log(
      "TOTAL: " +
        (
          subtotal +
          parseFloat(
            (subtotal * taxRate).toLocaleString(undefined, {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            })
          ) +
      ...
      transactions: [
        {
          ...
          amount: {
            ...
            details: {
              ...
              tax: (subtotal * taxRate).toLocaleString(undefined, {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              }),
              shipping: shipping.toLocaleString(undefined, {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              }),
            },
  ...
  paypal.payment.execute(
    ...
    function (error, payment) {
      if (error) {
        ...
        // `orders`.`email`
        db.query(
          `INSERT INTO orders(userid, status, date, total, first_name, last_name, address, address_2, city, state, zip, phone, email) VALUES (${userID}, 
                ${0}, NOW(), ${
            (subtotal +
              parseFloat(
                (subtotal * taxRate).toLocaleString(undefined, {
          ...
          (err, results, fields) => {
            ...
            coins.forEach((coin) => {
              ...
              db.query(
                `INSERT INTO order_items(orderid, coinid) VALUES (${results.insertId}, ${coin.id})`,
                (err, rows) => {
                  if (err) {
                    //This should never error, but if it does it's VERY bad. We took their money but never created an order for them.
                    //Maybe log errors like this to the db in the future just to be safe?
              ...
              //set coins as sold
              db.query(
                `UPDATE coins SET status = 2 WHERE id = ${coin.id}`,
                (err, rows) => {
                  if (err) {
                    //This should never error, but if it does it's VERY bad. We took their money but never created an order for them.
                    //Maybe log errors like this to the db in the future just to be safe?
...
router.post("/testsuccess", (req, res) => {
  ...
  db.query(`SELECT * FROM coins WHERE id in (${idsQuery})`, (err, rows) => {
    ...
    console.log("testtotal", testSubtotal);

    if (testSubtotal < minPurchase) {
      console.log("minpurch");
      return res.status(400).send({
        message:
          "Something went wrong. Make sure you've filled out all required fields.",
    ...
    console.log(
      ...
      (testSubtotal +
        parseFloat(
          (testSubtotal * taxRate).toLocaleString(undefined, {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })
        ) +
    ...
    db.query(
      `INSERT INTO orders(userid, status, date, total, first_name, last_name, address, address_2, city, state, zip, phone, email) VALUES (${getUserID(
        req
      )}, 
                    ${0}, NOW(), ${
        (testSubtotal +
          ...
          shipping) *
        100
      }, '${req.body.shipping.firstName}', '${req.body.shipping.lastName}', '${
        req.body.shipping.address
      }', '${req.body.shipping.address2}',
                    '${req.body.shipping.city}', '${
        req.body.shipping.state
      }', '${req.body.shipping.zip}', '${req.body.shipping.phone}', '${
        req.body.shipping.email
      }')`,
      (err, results, fields) => {
        if (err) {
        ...
        sendEmail(
          req.body.shipping.email,
          `Thank you for your purchase, ${req.body.shipping.firstName}!`,
          createReceiptEmail(
            req.body.shipping.firstName,
            coins,
            parseFloat(
        ...
        res.status(200).send({redirect: process.env.PAYPAL_SUCCESS_REDIRECT});
...
router.get("/cancel", (req, res) =>
  res.redirect(process.env.PAYPAL_FAILED_REDIRECT)
);
